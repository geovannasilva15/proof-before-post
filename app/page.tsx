...
import { buildReceiptSummary, downloadReceiptPdf, downloadReceiptPng, type ReceiptData } from "../lib/receipt";
...
export default function Home() {
  ...
  async function extractUrl() {
    ...
    try {
      ...
      const result = await response.json().catch(() => null) as { text?: string; code?: string } | null;
      ...
      setExtractionPreview(result.text); setExtractionState("ready");
    } catch { setExtractionError(t("extractionUnavailable")); setExtractionState("error"); }
  }

  function confirmExtraction() {
    setDraft(extractionPreview); setInputMode("text"); setExtractionState("idle"); setExtractionPreview(""); setAnalysis(null);
  }
  ...
  return (
    <main className="app-shell editor-page">
      ...
      {step === 1 && <>
        ...
        {inputMode === "url" && <section className="url-import-card"><label htmlFor="publication-url">{t("publicationUrl")}</label><div><input id="publication-url" type="url" value={urlInput} onChange={(event) => setUrlInput(event.target.value)} placeholder={t("publicationUrlPlaceholder")} /><button className="secondary-button" disabled={!urlInput.trim() || extractionState === "loading"} onClick={extractUrl}>{extractionState === "loading" ? t("extractingUrl") : t("extractUrl")}</button></div>{extractionError && <p className="field-error" role="alert">{extractionError}</p>}{extractionState === "ready" && <div className="extraction-preview"><strong>{t("extractionPreview")}</strong><textarea value={extractionPreview} onChange={(event) => setExtractionPreview(event.target.value)} /><div><button className="primary-button" onClick={confirmExtraction}>{t("confirmExtraction")}</button><button className="text-button" onClick={() => { setExtractionState("idle"); setExtractionPreview(""); }}>{t("cancelExtraction")}</button></div></div>}</section>}
