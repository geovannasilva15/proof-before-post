...
function analysisErrorMessage(language: Language, code: AnalysisErrorCode | null) {
  switch (code) {
    ...
    case "RATE_LIMITED":
      return tr(language, "Muitas pesquisas foram solicitadas deste dispositivo. Aguarde alguns minutos e tente novamente.", "Too many research requests were made from this device. Wait a few minutes and try again.");
    case "REFUSAL":
      return tr(language, "O serviço recusou analisar este conteúdo. Revise o rascunho ou tente outro texto.", "The service declined to analyze this content. Review the draft or try another text.");
    case "CLAIM_MISMATCH":
      return tr(language, "A afirmação pesquisada não pôde ser associada com segurança ao rascunho enviado. Tente novamente.", "The researched claim could not be matched safely to the submitted draft. Please try again.");
    case "INVALID_RESPONSE":
      return tr(language, "O serviço retornou uma resposta inválida. Nenhum resultado foi atribuído ao seu rascunho.", "The service returned an invalid response. No result was attached to your draft.");
    default:
...
export default function Home() {
  ...
  const sourceNote = noteForSource(sourceNotes, source?.id ?? "");
  ...
  const receiptDate = analysis?.searchedAt ? new Date(analysis.searchedAt) : new Date();
  const receiptDateText = new Intl.DateTimeFormat(language === "pt" ? "pt-BR" : "en-US", { dateStyle: "medium", timeStyle: "short" }).format(receiptDate);
  const receiptIsCoherent = isReceiptCoherent({ draft, revised, claimText: claim?.text ?? "", action, reflection });

  function applyDecisionState(state: ReviewDecisionState) {
    setSelectedClaimId(state.selectedClaimId);
  ...
  function chooseClaim(item: Claim) {
    applyDecisionState(stateAfterClaimSelection(item.id, item.sourceIds[0] ?? ""));
    narrator.stop();
  }

  function chooseSource(sourceId: string) {
    applyDecisionState(stateAfterSourceSelection({ selectedClaimId, selectedSourceId, support, action, revised, reflection }, sourceId));
    narrator.stop();
  }
  ...
  async function analyzeDraft() {
    ...
    try {
      const response = await fetch("/api/analyze", {
        ...
        body: JSON.stringify({ draft: submittedDraft, language: submittedLanguage }),
  ...
  function downloadReceipt() {
    ...
    ctx.font = "28px Arial";
    const laidOut = sections.map(([title, body]) => ({ title, body, bodyLines: wrapCanvasLines(ctx, body, 1020) }));
    let contentEnd = 245;
    for (const section of laidOut) {
      contentEnd += 38;
      contentEnd += section.bodyLines.length * 40;
      contentEnd += 54;
    }
    const footerHeight = 180;
    canvas.height = contentEnd + footerHeight + 60;
    ...
    ctx.font = "20px Arial";
    ctx.fillText(`${tr(language, "Pesquisa realizada em", "Research completed at")}: ${receiptDateText}`, 80, 205);
    let y = 245;
    for (const section of laidOut) {
    ...
    const footerTop = canvas.height - footerHeight - 40;
    ctx.fillStyle = "#101e3b";
    ctx.fillRect(70, footerTop, 1060, footerHeight);
    ctx.fillStyle = "#ffffff";
    ctx.font = "700 22px Arial";
    ctx.fillText(tr(language, "Este recibo não certifica que o conteúdo é verdadeiro.", "This receipt does not certify that the content is true."), 105, footerTop + 55);
    ctx.font = "19px Arial";
    ctx.fillStyle = "#cbd5e7";
    ctx.fillText(tr(language, "Ele documenta o processo de verificação do criador.", "It documents the creator's verification process."), 105, footerTop + 95);
    canvas.toBlob((blob) => {
      if (!blob) return;
  ...
  function continueFromDecision() {
    ...
    if (action === "research") {
      setAnalysis(null);
      applyDecisionState(stateAfterClaimSelection("", ""));
      setSourceNotes({});
      setNeedsResearchRefresh(true);
      setAnalysisError(null);
      setStep(1);
    ...
    setRevised(revisionForNextStep(draft, revised));
    if (reflection && reflectionForAction[action] !== reflection) setReflection("");
    setStep(5);
  }
  ...
  return (
    <main className="app-shell editor-page">
      <Header language={language} setLanguage={changeLanguage} home={goHome} compact />
      ...
      {step === 1 && <>
        ...
        <section className="editor-grid">
          <div className="draft-card">
            ...
            <textarea id="draft" aria-label={tr(language, "Seu rascunho", "Your draft")} value={draft} placeholder={tr(language, "Cole seu rascunho aqui…", "Paste your draft here…")} onChange={(event) => updateDraft(event.target.value)} disabled={researching} aria-busy={researching} autoFocus />
        ...
        <Actions back={goHome} language={language}>
      ...
      {step === 2 && analysis && <>
        ...
        <section className="claims-grid">{claims.map((item, index) => <article key={item.id} className={`claim-card ${item.tone} ${claim?.id === item.id ? "selected" : ""}`}><button className="claim-select" aria-label={tr(language, `Selecionar afirmação: ${item.text}`, `Select claim: ${item.text}`)} aria-pressed={claim?.id === item.id} onClick={() => chooseClaim(item)} /><div className="claim-head"><span>{String(index + 1).padStart(2, "0")}</span><b>{item.category}</b><i>{claim?.id === item.id ? "✓" : ""}</i></div><blockquote>“{item.text}”</blockquote><div className="claim-reason"><small>{tr(language, "POR QUE MERECE ATENÇÃO", "WHY IT NEEDS ATTENTION")}</small><p>{item.reason} <CitationLinks sourceIds={item.sourceIds} sources={analysis.sources} language={language} /></p></div><div className="claim-question"><span>?</span><p>{item.question}</p><button aria-label={tr(language, "Ouvir pergunta", "Listen to question")} title={tr(language, "Ouvir pergunta", "Listen to question")} onClick={() => narrator.speak(item.question)}>🔊</button></div></article>)}</section>
        <Actions back={() => setStep(1)} language={language}><button className="primary-button" onClick={() => { const firstSourceId = claim?.sourceIds[0] ?? ""; if (firstSourceId && firstSourceId !== selectedSourceId) { applyDecisionState(stateAfterSourceSelection({ selectedClaimId, selectedSourceId, support, action, revised, reflection }, firstSourceId)); } else { setSelectedSourceId(firstSourceId); } setStep(3); }}>{tr(language, "Examinar esta afirmação", "Examine this claim")}<span>→</span></button></Actions>
      </>}
      ...
      {step === 3 && analysis && claim && <>
        ...
        <section className="evidence-grid">
          <div className="evidence-panel">
            ...
            <div className="source-results">{relatedSources.map((item) => <button key={item.id} className={`source-result ${source?.id === item.id ? "selected" : ""}`} onClick={() => chooseSource(item.id)}><span>{item.publisher}</span><strong>{item.title}</strong><small>{item.publishedAt || tr(language, "Data não informada", "Date not provided")}</small></button>)}</div>
            {source && <SourceDetails source={source} language={language} />}
            <label className="source-notes">{tr(language, "SUAS ANOTAÇÕES SOBRE A FONTE", "YOUR NOTES ABOUT THE SOURCE")}<textarea value={sourceNote} onChange={(event) => setSourceNotes((notes) => updateSourceNote(notes, source?.id ?? "", event.target.value))} placeholder={tr(language, "Registre limitações, contexto ou dúvidas…", "Record limitations, context, or questions…")} /></label>
          </div>
      ...
      {step === 4 && <>
        ...
        <section className="decision-grid">{actionOptions[language].map((item) => <button key={item.id} className={action === item.id ? "selected" : ""} onClick={() => { setAction(item.id); setReflection(""); }}><span>{item.icon}</span><div><h3>{item.title}</h3><p>{item.description}</p></div><i>{action === item.id ? "✓" : "→"}</i></button>)}</section>
        ...
        <Actions back={() => setStep(3)} language={language}><button className="primary-button" disabled={!action} onClick={continueFromDecision}>{action === "research" ? tr(language, "Voltar e pesquisar novamente", "Return and research again") : tr(language, "Revisar o conteúdo", "Revise the content")}<span>→</span></button></Actions>
      </>}
      ...
      {step === 5 && <>
        ...
        <section className="reflection-card"><label>{tr(language, "O que mudou na sua conclusão?", "What changed in your conclusion?")}</label><div>{reflectionOptions[language].map((item) => { const allowed = Boolean(action) && reflectionForAction[action as ActionId] === item.id; return <button key={item.id} className={reflection === item.id ? "selected" : ""} disabled={!allowed} aria-disabled={!allowed} onClick={() => setReflection(item.id)}>{reflection === item.id ? "✓ " : ""}{item.label}</button>; })}</div></section>
        <Actions back={() => setStep(4)} language={language}><button className="primary-button" disabled={!receiptIsCoherent} onClick={() => setStep(6)}>{tr(language, "Criar meu Recibo de Evidências", "Create my Evidence Receipt")}<span>→</span></button></Actions>
      </>}
